#!/bin/bash
# Install the AppArmor profile for win11-clipboard-history.
#
...
# To enforce it, run: sudo aa-enforce /etc/apparmor.d/win11-clipboard-history
#
# Usage: sudo ./install.sh [--enforce]

set -e

PROFILE_SRC="$(dirname "$0")/win11-clipboard-history"
PROFILE_DEST="/etc/apparmor.d/win11-clipboard-history"
...
if [ "${1:-}" = "--enforce" ]; then
    ...
    echo "AppArmor profile installed and ENFORCED for win11-clipboard-history."
    ...
    echo "To enforce: sudo aa-enforce /etc/apparmor.d/win11-clipboard-history"
    echo "To remove:  sudo aa-remove /etc/apparmor.d/win11-clipboard-history"
fi
