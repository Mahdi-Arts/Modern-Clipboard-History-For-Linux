...
//! Full-resolution PNG lives on disk; only a small thumbnail is sent over IPC.
...
/// Persist a clipboard image as PNG and return a compact thumbnail.
pub fn store_rgba(images_dir: &Path, id: &str, image: &ImageData<'_>) -> Result<StoredImage, String> {
    ...
    let full_path = images_dir.join(format!("{id}.png"));
    let tmp = full_path.with_extension("png.tmp");
    dyn_img
        .save_with_format(&tmp, ImageFormat::Png)
        .map_err(|e| format!("Failed to encode PNG: {e}"))?;
    fs::rename(&tmp, &full_path).map_err(|e| format!("Failed to move PNG into place: {e}"))?;
...
pub fn store_png_bytes(images_dir: &Path, id: &str, png: &[u8]) -> Result<StoredImage, String> {
    fs::create_dir_all(images_dir).map_err(|e| format!("Failed to create images dir: {e}"))?;
    ...
    let full_path = images_dir.join(format!("{id}.png"));
    crate::fs_atomic::write_atomic(&full_path, png).map_err(|e| e.to_string())?;
    crate::fs_atomic::restrict_permissions(&full_path);
...
pub fn read_png(path: &Path) -> Result<Vec<u8>, String> {
    fs::read(path).map_err(|e| format!("Failed to read stored image {}: {e}", path.display()))
}
