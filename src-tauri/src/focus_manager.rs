...
pub fn save_focused_window() {
    ...
    match crate::paste_sync::focused_window() {
        Some(window_id) => {
            ...
            etracing::info!("[FocusManager] Saved focused window: {}", window_id);
        }
        None => etracing::info!("[FocusManager] Failed to query the focused X11 window"),
    }
}
...
pub fn restore_focused_window() -> Result<bool, String> {
    ...
    etracing::info!("[FocusManager] Restoring focus to window: {}", window_id);
...
pub fn wait_for_window_by_title(title: &str, timeout: Duration) -> Option<u32> {
    ...
    etracing::info!("[FocusManager] Timeout waiting for window '{}'", title);
    None
}
...
pub fn x11_force_input_focus(window_id: u32) -> Result<(), String> {
    ...
    etracing::info!("[FocusManager] Forced input focus to window {}", window_id);
    Ok(())
}
    ...
    match get_focused_window() {
        Some(current_focus) => {
            if current_focus != window_id {
                etracing::info!("[FocusManager] Focus not acquired, forcing input focus");
