...
pub fn init() {
    ...
    if let Err(error) = std::thread::Builder::new()
        ...
        .spawn(warmup)
    {
        etracing::info!("[SimulatePaste] Failed to start warmup thread: {}", error);
    }
}
...
fn warm_up_xtest() {
    ...
    match XtestDevice::create() {
        Ok(created) => {
            *device = Some(created);
            etracing::info!("[XTest] Persistent X11 input client is ready");
        }
        Err(error) => etracing::info!(
...
fn warm_up_uinput() {
    ...
    match UinputDevice::create() {
        Ok(created) => {
            *device = Some(created);
            etracing::info!("[uinput] Virtual keyboard warmed up at startup");
        }
        Err(error) => etracing::info!(
...
pub fn simulate_paste_keystroke() -> Result<(), String> {
    etracing::info!("[SimulatePaste] Sending Ctrl+V...");
    ...
    for (name, strategy) in strategies {
        match strategy() {
            Ok(()) => {
                etracing::info!("[SimulatePaste] Ctrl+V sent via {}", name);
                return Ok(());
            }
            Err(PasteFailure::Retryable(error)) => {
                etracing::info!("[SimulatePaste] {} unavailable: {}", name, error);
            }
...
impl XtestDevice {
    ...
    fn refresh_keymap_if_needed(&mut self) -> Result<(), String> {
        ...
        if mapping_changed {
            ...
            etracing::info!("[XTest] Keyboard mapping changed; refreshed paste keycodes");
        }
...
impl UinputDevice {
    fn create() -> Result<Self, String> {
        ...
        etracing::info!("[uinput] Persistent virtual keyboard is ready");
...
fn simulate_paste_uinput() -> Result<(), PasteFailure> {
    ...
    if let Some(existing) = device.as_mut() {
        match existing.send_ctrl_v() {
            Ok(()) => return Ok(()),
            Err(error) => {
                etracing::info!("[uinput] Existing device failed: {}", error);
                *device = None;
